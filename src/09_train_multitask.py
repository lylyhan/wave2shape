...
def create_model_adjustable(J,Q,order,k_size,nchan_out,activation):
	...
	model.add(Dense(7, activation=activation)) #output layer that corresponds to the 5 physical parameters.
...
save_log = os.path.join(trial_dir,pickle_name+"_multitask_score.pkl")
#model_adjustable.summary()
print('Start fitting the model...')
for epoch in range(30):
	np.random.shuffle(idx)
	Sy_temp = Sy_train_log2[idx[:m],:shape_time,:]
	y_temp = y_train_7[idx[:m],:]
	...
	hist = model_adjustable.fit(Sy_temp,
				...
				verbose=2,
				batch_size=bs,
				validation_data = (Sy_val_log2[:,-shape_time:,:],y_val_7),
				use_multiprocessing=False)

	validation_loss = hist.history['val_loss'][0]

	test_loss.append(model_adjustable.evaluate(Sy_test_log2,y_test_7)[0])
	...
	if validation_loss < best_validation_loss:
		...
		epoch_network_path = os.path.join(
		   trial_dir, "_".join([ "J-" + str(J).zfill(2), "Q-" + str(Q).zfill(2), "order" + str(order)]) + "_multitask.h5")
