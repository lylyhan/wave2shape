...
def set(args=None):
    ...
    name = 'wav2shape_ploss_wpitch',
    purpose = 'multitask regression experiments with parameter loss variations',
    author = 'han han',
    ...
    experiment.add_plan('ploss',
                       feature_type = ['cqt'],
                       J = [10],
                       Q = [12],
                       loss_type = ["ploss"],
                       param = ['alpha'],
                       pitchmode = ['wpitch','wopitch'],
                       activation_type = ['linear'],
    )
    experiment.add_plan('weighted_p',
                       feature_type = ['cqt'],
                       J = [10],
                       Q = [12],
                       loss_type = ["weighted_p"],
                       weight_type = ["nn_limit","grad"],#nn_limit, grad, nn_lsq
                       param = ['alpha'],
                       pitchmode = ['wpitch','wopitch'],
                       activation_type = ['linear'],
    )
...
def step(setting, experiment):
    ...
    pitchmode = setting.pitchmode
    
    if loss == "weighted_p":
        weighting = setting.weight_type
        #weight_nbr = setting.weight_nnbr
        #weight_setup = {"type":weighting,"n_nbr":weight_nbr[0],"n":weight_nbr[1]}
        weight_setup = {"type":weighting,"n_nbr":10,"n":5}
    else:
        weight_setup = None
