...
def create_model_adjustable(J,Q,order,k_size,nchan_out,activation):
    ...
    x = FiLM(input_shape = [Sc_input_shape,u_input_shape],name='FiLM_layer',dynamic=True)([Sc_input_batched,u_input])
...
save_log = os.path.join(trial_dir,pickle_name+"_film_bf_score.pkl")
...
for epoch in range(30):
	...
	if validation_loss < best_validation_loss:
		...
		epoch_network_path = os.path.join(
		   trial_dir, "_".join([ "J-" + str(J).zfill(2), "Q-" + str(Q).zfill(2), "order" + str(order)]) + "_film_bf.h5")
