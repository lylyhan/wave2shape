...
def get_gaus_f(m,l,tau,r,gauss_sigma):
    #trapezoid rule to integrate f(x)sin(mpix) from 0 to l
    #(f(a)+f(b))*(b-a)/2
    integral = 0
    x = approxnorm(l,l*r,gauss_sigma,tau)
    h = l/tau
    for i in range(tau):
        #x(i+2)
...
def getsounds_imp_gaus(m1,m2,r1,r2,w11,tau11,p,D,alpha,sr,gauss_sigma):
    """
    ...
    for i in range(m1):
        for j in range(m2):
            ...
            k[i,j] = get_gaus_k(i+1,j+1,omega[i,j],get_gaus_f(i+1,1,300,r1,gauss_sigma),get_gaus_f(j+1,alpha,300,r2,gauss_sigma),x1,x2,l,alpha) # the covered striking length is 1
...
if __name__ == '__main__':
    sr=22050
    y = getsounds_imp_gaus(20,20,0.5,0.5,2000,0.1,0.2,0.1,0.7,sr,0.2)
